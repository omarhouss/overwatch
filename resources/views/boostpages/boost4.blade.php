			...
			<div class="flex flex-col lg:gap-[70px] gap-[30px]">
				<div class="flex">
					...
					<h1 id="h1-style-mb" class=" text-[#E94A65] xl:text-[#ffff] sm:mx-0 mx-auto font-[Manrope] font-normal font-[700] text-[18px] sm:text-[22px]">
						<span class="text-[#ffff] font-[Manrope] font-normal font-[700] ">SELECT  YOUR  </span>AMOUNT OF WINS 
					</h1>
				</div>
...
<script>
    ...
    function calculateTotalPrice() {
        const wins = numberOfWins; // Use the numberOfWins variable directly0;
        const basePrice = 1.75 * wins;
...
purchaseButton.addEventListener('click', async () => {
  ...
  const service = "Quick Play";
  const response = await fetch('/order4', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
    },
    body: JSON.stringify({ wins, total_price,service }),
